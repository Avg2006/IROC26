...
def nearest_point_on_line(px, py, line):
    x1, y1, x2, y2 = line
    dx = x2 - x1
    dy = y2 - y1
    if dx == 0 and dy == 0:
...
class CombinedPerceptionNode(Node):
    ...
    def __init__(self):
        ...
        self.max_image_counter_path = os.path.expanduser("~/iroc_2026-scripts/max_image_counter.yaml")
        self.max_image_counter = 1
        self.load_max_img(self.max_image_counter_path)
        ...
        self.lower_yellow = np.array([11, 66, 135])
        self.upper_yellow = np.array([39, 255, 255])
        ...
        self.boundary_pub = self.create_publisher(Bool, '/boundary_detected', 10)
        self.boundary_offset_pub = self.create_publisher(Point, '/boundary_offset', 10)
        self.front_back_offset_pub = self.create_publisher(Point, '/front_back_offset', 10)
        self.side_line_offset_pub = self.create_publisher(Point, '/side_line_offset', 10)
        self.is_front_back_pub = self.create_publisher(Bool, '/boundary_is_front_back', 10)
        ...
        self.corner_pub = self.create_publisher(Bool, '/corner_detected', 10)
        self.corner_type_pub = self.create_publisher(String, '/corner_type', 10)


        # Shared Image Publisher
        self.image_pub = self.create_publisher(CompressedImage, "aruco_debug_image/compressed", qos_profile_sensor_data)

        # Height Subscriber
        self.height_sub = self.create_subscription(PoseStamped, '/reference_odom', self.height_callback,
    ...
    # ---------------- YAML LOADER ---------------- #
    def load_max_img(self, file_path):
        try:
            with open(file_path, "r") as file:
                counter_data = yaml.safe_load(file)
                self.max_image_counter = counter_data["count"]
                self.get_logger().info(f"Successfully read the counter {self.max_image_counter}")
        except Exception as e:
            self.get_logger().error(f"Failed to load max_counter from {file_path}. Error: {e}")

    def update_max_counter(self):
        data = {
            "count": self.max_image_counter,
        }
        try:
            with open(self.max_image_counter_path, "w") as file:
                yaml.dump(data, file)
                self.get_logger().info(f"Max Count updated : {self.max_image_counter}")
        except Exception as e:
    ...
    def height_callback(self, msg):
        ...
        # self.get_logger().info(f"x: {self.current_x}, y:{self.current_y}, z:{self.drone_height_meters}")
    ...
    def take_picture_cb(self, msg):
        ...
        if current_signal and not self.picture_trigger_state:
            for _ in range(0, 10):
                ...
                filename = os.path.join(self.save_dir, f"image_{self.max_image_counter}.jpg")
                self.max_image_counter += 1
    ...
    def lines_are_similar(self, line1, line2, center_x, center_y):
        ...
        parallel_distance_threshold = (
                                              self.parallel_dist_thresh_meters * self.focal_length_pixels) / self.drone_height_meters
    ...
    def classify_corner(self, line1, line2, corner):
        ...
        if d1 > d2:
            hvx, hvy = hx1 - px, hy1 - py
        else:
            hvx, hvy = hx2 - px, hy2 - py
        ...
        if d1 > d2:
            vvx, vvy = vx1 - px, vy1 - py
        else:
            vvx, vvy = vx2 - px, vy2 - py
    ...
    def process_line_detection(self, frame):
        ...
        line_detected = True if len(merged_lines) > 0 else False
        ...
        for i in range(len(merged_lines)):
            for j in range(i + 1, len(merged_lines)):
                ...
                ang_diff = angle_difference(line_angle(*line1), line_angle(*line2))
                # Corner lines must be roughly perpendicular (e.g. between 65 and 115 degrees)
                if ang_diff < 65 or ang_diff > 115:
                    continue
                ...
                if 0 <= px < width and 0 <= py < height:
                    # Check that the intersection is close to both actual line segments
                    nx1, ny1 = nearest_point_on_line(px, py, line1)
                    nx2, ny2 = nearest_point_on_line(px, py, line2)
                    dist1 = math.hypot(nx1 - px, ny1 - py)
                    dist2 = math.hypot(nx2 - px, ny2 - py)
                    
                    # Both distances must be within 50 pixels to be a physical corner
        ...
        if len(corners) > 0:
            ...
            if stable_result is not None:
                ...
                cv2.putText(frame, "CORNER", (sx + 15, sy - 15),
        ...
        if closest_line_parallel is not None:
            parallel_distance_meters = (
                                               min_distance_pixels_parallel * self.drone_height_meters) / self.focal_length_pixels
        if closest_line_perpendicular is not None:
            perpendicular_distance_meters = (
                                                    min_distance_pixels_perpendicular * self.drone_height_meters) / self.focal_length_pixels

        # Calculate vector to front/back line (closest_line_parallel)
        parallel_offset = (0.0, 0.0)
        parallel_angle = 0.0
        if closest_line_parallel is not None:
            nx, ny = nearest_point_on_line(center_x, center_y, closest_line_parallel)
            vx_pix = nx - center_x
            vy_pix = ny - center_y
            offset_x_m = (vx_pix * self.drone_height_meters) / self.focal_length_pixels
            offset_y_m = (vy_pix * self.drone_height_meters) / self.focal_length_pixels
            parallel_offset = (offset_x_m, offset_y_m)
            x1, y1, x2, y2 = closest_line_parallel
            parallel_angle = math.atan2(y2 - y1, x2 - x1)

        # Calculate vector to side line (closest_line_perpendicular)
        perpend_offset = (0.0, 0.0)
        perpend_angle = 0.0
        if closest_line_perpendicular is not None:
            nx, ny = nearest_point_on_line(center_x, center_y, closest_line_perpendicular)
            vx_pix = nx - center_x
            vy_pix = ny - center_y
            offset_x_m = (vx_pix * self.drone_height_meters) / self.focal_length_pixels
            offset_y_m = (vy_pix * self.drone_height_meters) / self.focal_length_pixels
            perpend_offset = (offset_x_m, offset_y_m)
            x1, y1, x2, y2 = closest_line_perpendicular
            perpend_angle = math.atan2(y2 - y1, x2 - x1)

        # Which direction the currently-visible boundary line lies in.
        # True  -> the line is the front/back (image-parallel) line
        # False -> the line is the side (image-perpendicular) line
        # Only one of closest_line_parallel / closest_line_perpendicular is
        # expected to be populated at a time; if perpendicular is the only
        # one found, report False, otherwise default True.
        is_front_back = closest_line_perpendicular is None

        return line_detected, parallel_distance_meters, perpendicular_distance_meters, corner_detected, corner_type, parallel_offset, parallel_angle, perpend_offset, perpend_angle, is_front_back
    ...
    def timer_callback(self):
        ...
        offset_msg = Point()
        front_back_offset_msg = Point()
        side_offset_msg = Point()
        is_front_back_msg = Bool()
        if not self.mission_complete:

            # 1. Process Boundary/Lines (Modifies cv_image by drawing lines)
            detected, dist1, dist2, c_detected, corner_type, par_offset, par_angle, perp_offset, perp_angle, is_front_back = self.process_line_detection(cv_image)

            # Update Values
            detected_msg.data = detected
            d1_msg.data = float(dist1)
            d2_msg.data = float(dist2)
            c_msg.data = c_detected
            c_type.data = corner_type

            if par_offset[0]**2 + par_offset[1]**2 < perp_offset[0]**2 + perp_offset[1]**2:
                if par_offset[0]**2 + par_offset[1]**2 == 0:
                    nearest_offset = perp_offset
                    nearest_angle = perp_angle
                else:
                    nearest_offset = par_offset
                    nearest_angle = par_angle
            else:
                if perp_offset[0]**2 + perp_offset[1]**2 == 0:
                    nearest_offset = par_offset
                    nearest_angle = par_angle
                else:
                    nearest_offset = perp_offset
                    nearest_angle = perp_angle
            offset_msg.x = float(nearest_offset[0])
            offset_msg.y = float(nearest_offset[1])
            offset_msg.z = float(nearest_angle)

            front_back_offset_msg.x = float(par_offset[0])
            front_back_offset_msg.y = float(par_offset[1])
            front_back_offset_msg.z = float(par_angle)

            side_offset_msg.x = float(perp_offset[0])
            side_offset_msg.y = float(perp_offset[1])
            side_offset_msg.z = float(perp_angle)

            is_front_back_msg.data = bool(is_front_back)
            ...
            offset_msg.y = 0.0
            offset_msg.z = 0.0
            front_back_offset_msg.x = 0.0
            front_back_offset_msg.y = 0.0
            front_back_offset_msg.z = 0.0
            side_offset_msg.x = 0.0
            side_offset_msg.y = 0.0
            side_offset_msg.z = 0.0
            is_front_back_msg.data = True

        self.boundary_pub.publish(detected_msg)
        self.boundary_offset_pub.publish(offset_msg)
        self.front_back_offset_pub.publish(front_back_offset_msg)
        self.side_line_offset_pub.publish(side_offset_msg)
        self.is_front_back_pub.publish(is_front_back_msg)
        self.line1_dist_pub.publish(d1_msg)
        self.line2_dist_pub.publish(d2_msg)
        self.corner_pub.publish(c_msg)
        self.corner_type_pub.publish(c_type)
...
def main(args=None):
    ...
    try:
        ...
        node.get_logger().info("Attempting to save maxcounter")
        node.update_max_counter()
    finally:
        node.destroy_node()
        if rclpy.ok():
            rclpy.shutdown()

...
from drone_interfaces.srv import TransferPhoto
...
class droneControl(Node):
    def __init__(self):
        ...
        self.transfer_client = self.create_client(TransferPhoto, 'transfer_photo')

        # Photo transfer config
        self.laptop_ip     = '192.168.0.100'
        self.ssh_user      = 'kanak'
        self.ssh_pass      = 'your_password'
        self.remote_path   = '/home/kanak/iroc_2026_images_rcv/'
        self.local_path    = '~/iroc_2026_images/'
        ...
        self.waypoints = [
            ...
            (-1.0,   1.0,  2.5),   
            (-1.0,  0.0,  2.5),   
            (-2.0,   0.0,  2.5),   #  8  back to centre
            (-2.0,   1.0,  2.5),   #  9  (reuse centre — matches original count)
            (-2.0,  2.0,  2.5),   # 10  climb/down to 2.0 m
            (-2.0,   3.0,  2.5),
            (0.0,   0.0,  2.5),   # 16  ← LAST WAYPOINT — return home / ArUco search
        ]
    ...
    def _transfer_cb(self, future):
        result = future.result()
        if result and result.success:
            self.get_logger().info(f'Transfer succeeded: {result.message}')
        else:
            self.get_logger().error(f'Transfer failed: {result.message}')
    ...
    def loop(self):
        ...
        # ── ARUCO_ALIGN ───────────────────────────────────────────────────────
            ...
            error_x = max(self.error_x / 2, -0.1) if self.error_x < 0 else min(self.error_x / 2, 0.1)
            error_y = max(self.error_y / 2, -0.1) if self.error_y < 0 else min(self.error_y / 2, 0.1)
        ...
        # ── ARUCO_HOVER_ALIGNED ───────────────────────────────────────────────
            ...
            if elapsed >= self.hover_duration:
                if self.current_pos[2] <= self.land_alt:
                    ...
                    new_alt = max(self.current_pos[2] - self.descend_step, 0.55)
        ...
        # ── LAND (precision land after descent steps) ─────────────────────────
            ...
            self.land()  # sets flight mode to LAND, phase → TRANSFER_PHOTOS

        # ── TRANSFER_PHOTOS ───────────────────────────────────────────────────
        elif self.phase == "TRANSFER_PHOTOS":
            if not self.transfer_done:
                self.transfer_done = True
                self.get_logger().info("Drone landed — starting photo transfer…")

                req = TransferPhoto.Request()
                req.laptop_ip        = self.laptop_ip
                req.username         = self.ssh_user
                req.password         = self.ssh_pass
                req.remote_path      = self.remote_path
                req.local_image_path = self.local_path

                future = self.transfer_client.call_async(req)
                future.add_done_callback(self._transfer_cb)
                self.phase = "DONE"

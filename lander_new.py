...
class droneControl(Node):
    ...
    def aruco_callback(self, msg):
        if msg.poses:
            ...
            self.last_aruco_global = [
                ...
                self.current_pos[2],              # always recover at cruise altitude, not wherever we lost it
            ]
    ...
    def loop(self):
        ...
        # ── HOVER_ALIGNED ─────────────────────────────────────────────────────
            ...
            # Hold ArUco XY (not drifted current pos) at current altitude
            # if self.last_aruco_global is not None:
            #     hold_target = [
            #         self.last_aruco_global[0],
            #         self.last_aruco_global[1],
            #         self.current_pos[2],
            #     ]
            # else:
            #     hold_target = [self.current_pos[0], self.current_pos[1], self.current_pos[2]]
            ...
            if elapsed >= self.hover_duration:
                if self.current_pos[2] <= self.land_alt:
                    self.get_logger().info("Below land threshold → LAND (pre-align check)")
                    self.phase = "LAND"
                else:
        ...
        # ── LAND ──────────────────────────────────────────────────────────────
        # Pre-land alignment gate: re-align over marker before committing to LAND mode.
        # Only calls self.land() once ArUco is centred within align_threshold.
        elif self.phase == "LAND":
            # if not self.aruco_detected:
            #     # Marker lost right before land — hold last known XY and wait
            #     if self.last_aruco_global is not None:
            #         self.hold([
            #             self.last_aruco_global[0],
            #             self.last_aruco_global[1],
            ...
            #         "LAND: ArUco not visible, going to last aruco know position",
            #         throttle_duration_sec=1.0
            #     )
            #     # return

            # # Marker visible — drive toward it
            # target = [
            ...
            # self.hold(target)
            # self.get_logger().info(
            #     f"Pre-land align | err_x={self.error_x:+.3f}  err_y={self.error_y:+.3f}  "
            #     f"alt={self.current_pos[2]:.2f}"
            # )

            # if self.xy_aligned():
            #     self.get_logger().info("Pre-land alignment confirmed → LAND mode\a\a\a")
            self.land()

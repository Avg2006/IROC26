...
class WaypointFullMission(Node):
    def __init__(self):
        ...
        # ── ArUco detection (scanned for on the way home) ───────────────────
    ...
    def loop(self):
        ...
        # ── RETURN_HOME ──────────────────────────────────────────────────
            ...
            if self.aruco_detected:
                self.get_logger().info("ArUco spotted en-route home -> ARUCO_ALIGN")
            ...
            if self.arrived(self.home_position):
                self.get_logger().info("Reached home, no ArUco seen -> landing")
                self.phase = "LAND"
                return
        ...
        # ── ARUCO_ALIGN ────────────────────────────────────────────────────
            ...
            if not self.aruco_detected:
                if self.last_aruco_global is not None:
                    ...
                    self.get_logger().warn("ArUco lost, no global target -> RETURN_HOME")
                    self.phase = "RETURN_HOME"
                    self.start_time = time.time()
                return
            ...
            # Clamp correction to +-0.05 m per tick
        ...
        # ── ARUCO_RECOVER ────────────────────────────────────────────────────
            ...
            if self.arrived(self.last_aruco_global):
                self.get_logger().info(
                    "Reached last known ArUco pos, still no marker -> RETURN_HOME"
                )
                self.phase = "RETURN_HOME"
                self.start_time = time.time()
